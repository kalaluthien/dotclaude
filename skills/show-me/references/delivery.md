...
- A `dashboard` at a run-named or `/tmp` path is one nobody returns to, which
  makes it a `view` of the numbers.
- Every HTML page passes the 320 px probe ([to-png](to-png.md)) before it is
  delivered (C7); a `dashboard` also passes its desktop-width run (V3).
