...
- **Ext**: 0 URLs, other than `data:`, `about:`, `blob:` or a `#` fragment,
  among: every `src`, `poster` and `object` `data`; every `srcset`
  candidate, split as the spec splits it; the `href` of every `link` and
  every SVG `image` and `use`; every `url()` and `@import` in the style
  sheets, imported ones included, and in `style` attributes, read as the
  browser parsed them (`cssText`: no comments, strings skipped, `@namespace`
  aside); and what the page fetched before `load` (Resource Timing, which
  lists no `file://` fetch). An empty value is skipped; a URL that does not
  parse counts. § Page: One file fetches nothing. Any other shape is not
  read: an `xlink:href`, an `feImage` `href`, a legacy `background`
  attribute, a `link` `imagesrcset`, an SVG presentation attribute's
  `url()`, an SVG `script` `href`, a shadow root's content, a URL a script
  builds, a nested frame's own fetches.
