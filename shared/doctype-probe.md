...
- **Motion**: 0, the larger of the two readings, each the page's
  ...
  running at that moment -- plus its SVG `animate`, `animateMotion`,
  ...
  transition on `:hover`, or on `:focus` of anything but a scroll box; an
  animation a script starts after load; `scroll-behavior: smooth`; a GIF, a
  video, a canvas, a style a script moves on a timer.
