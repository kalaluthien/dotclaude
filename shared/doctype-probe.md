...
var r=e.scrollWidth+'/'+e.clientWidth+' '+s.map(function(u){return u<1/0?u.toFixed(1)+'px':'-'}).join(' '),k=e.scrollWidth==e.clientWidth&&F.length==2&&s[0]>=F[0]&&s[1]>=F[1]&&!i&&!q&&!N&&K=='same';
...
if(b.left<o.left-.5||b.right>o.right+.5||b.top+a<o.top-.5||b.bottom-j>o.bottom+.5)u=1});v+=u});document.body.dataset.r=r+' '+x+'/'+y+' clip '+v+' id '+i+' ext '+q+' motion '+N+' skin '+K+' floor '+F+(k&&!x&&!v?' pass':' FAIL')};f.src=location.hash.slice(1)</script>
EOF
for P in <page>...; do
  K=differs
...
- **Motion**: 0, the larger of two readings, one on load and one after the
  clicks: the page's `getAnimations()` -- CSS animations, CSS transitions and
  script animations -- plus its SVG `animate`, `animateMotion`,
  `animateTransform` and `set`, all under `--force-prefers-reduced-motion`
  (`doctype-page.md`: Motion). Not read: motion under `no-preference` alone, a
  GIF, a video, a canvas, a style a script moves on a timer.
