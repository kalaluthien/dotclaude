...
- Name a resource for the work it is for, never for the settings it was created with. A name is read to route work, not to run it: a setting in the name answers no question the reader has and goes stale as soon as the setting changes, while a name that states the role lets one documented mapping carry every setting the role implies.
