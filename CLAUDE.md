...
3. Pick the model by task depth and the effort by task breadth. Work whose approach is not yet clear — an extremely hard problem, a request that reads two ways, a design with no obvious shape, a failure nothing has explained — runs Fable. Work that decides against an approach already clear — ideation, planning, implementation, review — runs Opus; work that retrieves or condenses knowledge — search, classification, summarization — runs Sonnet. Effort is a thinking budget, and both breadth and difficulty spend it: high or xhigh for a task that spans many exceptional cases or a large state space, or for one whose single case is genuinely hard; low or medium for a task both narrow and well understood. Where the two disagree, take the higher — a session started too low meets the hard part with nothing left. Where the work is a backlog item, its `#easy` or `#hard` tag is that estimate already made, so read it instead of guessing again. Fable is chosen by the task and never by position: launch it below the orchestrator only when difficulty or ambiguity is the reason, and never because the orchestrating session happens to run it. When the orchestrating session runs a tier costing more per turn than Opus, it delegates its own hands-on editing and debugging rather than doing them, since hands-on work multiplies turns; when it runs Opus, it may do narrow hands-on work itself and still delegates anything wide enough to need its own context.
...
- A regression test earns trust only by failing first: break the behaviour in the source, watch the named test fail, then restore. A test authored against an already-fixed tree can pass for reasons unrelated to the defect. Restore by *undoing the one edit you made*, never with a version-control revert of the whole file — `git stash`, `git checkout --` and their kin restore the file to its last commit, which on a working tree carrying other uncommitted work silently discards all of it, and the loss is invisible because the suite goes green either way. Commit first where you can; otherwise mutate and unmutate by exact string, and re-read the line to confirm the restore landed.
...
The memtypes divide by what would make the file wrong. Nothing falsifies `history-`, so it only grows, and it records what version control cannot: changes to unversioned things, and rejected options with their kill reasons. A `backlog` item dies when its work closes. A `topic-` file states the current truth of one subject, distilled from work or from a `history-` log: it is updated in place to hold only what is true now, never the events that made it true — the paired `history-` file keeps those. It splits from `setup-` by recovery cost, not by subject: a `setup-` fact is one probe away, a `topic-` truth was bought by analysis or accumulated decisions and no probe re-derives it. It stays out of two other homes: external technical knowledge belongs to the notes wiki, and a repository's own truth belongs to that repository's `docs/`. A `setup-` fact is re-derivable by probing the tool, so losing one costs a probe; a `feedback-` rule was given by the owner — a correction bought with an observed failure, or a standing fact about who the owner is and what they prefer — and is not re-derivable, so losing one repeats the failure or re-asks the owner. A decision is not a memtype: the choice goes to `history-` when its evidence would otherwise vanish, and the chosen rule lives where rules of its kind live. A file's frontmatter `description` and its `MEMORY.md` line state the file's role, never its contents: an enumerated content list is a mirror that goes stale on the file's next edit.
