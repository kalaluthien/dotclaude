...
## Claude Code
1. A project's instructions live in `PROJECT/AGENTS.md`, and `PROJECT/.claude/CLAUDE.md` imports it with `@../AGENTS.md` so every harness reads one source — the workspace root included. The import expands only for a session started at that repository root; read as an ancestor or from a subdirectory it is inert text.
