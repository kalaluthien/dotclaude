...
Write an item's body as indented field bullets, nothing after the bold title on the head line. Fields in order: `what:` (the work in one plain sentence), `why:` (why it exists), `next:` (the next step and who takes it), optional `scope:` (what the worker touches) and `evidence:` (paths, commits, dates). The reviewing owner reads `what`/`why`/`next`; the working session reads `what`/`scope`/`evidence`. Each field is bounded by `item.body.field_max_chars`, and a bound is a ceiling, never a target — the superseded single 300-character budget taught sessions to fill it. Use plain words and name the actor: the row is read as a card on a phone. Rewriting a row preserves every fact, marker, date, tag, and reference. (Owner decision, 2026-08-15, superseding the 2026-08-12 three-sentence body; record in workspace git history — `docs/agent-backlog-writing.html`, deleted at cf5e1b4 — and in the workspace pool's `feedback-backlog-wording`.)
    ...
    "body": {
      ...
      "fields": [
        ...
        { "label": "why", "required": true, "reader": "reviewer",
          "means": "why the item exists" },
        { "label": "next", "required": true, "reader": "reviewer",
          "means": "the next step and who takes it" },
        { "label": "scope", "required": false, "reader": "worker",
          "means": "what the worker touches" },
        { "label": "evidence", "required": false, "reader": "worker",
          "means": "the paths, commits, and dates that back the other fields" }
      ]
    },
    "markers": [
      { "marker": " ", "status": "open" },
