...
Write every item in a `backlog.md` task-bearing section as `- [<m>] YYYY-MM-DD #tag … **Title.** body`, where `<m>` is one of ` ` (open) or `/` (working), the date is the day the item was created, never the day it moved, and each `#tag` sits between the date and the title. The marker is the item's state; a tag is an orthogonal annotation that combines with either state. A tag says one of two things, and which one it says is declared, never inferred from the word. A **waiting** tag names who the item waits on — `#need-you` waits on the owner. A **scope** tag names the kind of change the item expects — `#agent`, `#code`, `#docs`, `#test` — and waits on nobody: it is informational and holds nothing. Every item carries at least one scope tag (owner decision, 2026-08-08, superseding "leave it untagged where the kind is still open"): the tag shows an item's reach before anyone opens it, and on a `#need-you` decision row it shows the owner what confirming would change. A survey or study is `#docs` — its deliverable is a written report. When the kind is genuinely open, tag the likeliest scope and let the work correct it; a missing tag hides the reach from every reader, which costs more than a corrected guess. An owner-waiting tag holds the item's next automatic transition: the launch while the item is open, the close while it is working, so an agent never starts a `[ ] #need-you` row and never deletes a `[/] #need-you` row. Read the pair together: `[/] #need-you` means the work is finished and the close waits on the owner, `[ ] #need-you` means the next step is the owner's. A prose section — Why, How to apply, Horizon, Preliminary research — keeps plain bullets. One fixed grammar lets a reader and a tool read the same file, a waiting tag separates what is waiting from what is merely open, a scope tag says what kind of work the item is without anybody opening it, and the creation date makes an item's age visible without a session log. A `PostToolUse` hook, `~/.claude/hooks/check-backlog-format.py`, rejects a write that breaks the grammar.
