...
Write the text after the bold title as at most three short sentences, in one order: what the work is, why it exists, what happens next. Put evidence — a path, a test name, a commit, a date — in parentheses at the end of the clause it supports, never in the spine of a sentence. Use plain words and name the actor, because the row is read by the owner as a card on a phone and not by the session that wrote it, so an internal state name or a nested clause blocks that read. The text after the title has a budget of 300 characters, declared as `item.body.max_chars` in the contract below, and the same hook rejects a row that runs past it — the budget catches overflow and the three sentences carry the reading, so a row under budget can still break the rule. Rewriting a row preserves every fact, marker, date, tag, and reference, and changes the wording alone. (Owner decision, 2026-08-12; the options and the kill reasons are in `~/workspace/docs/agent-backlog-writing.html`.)
