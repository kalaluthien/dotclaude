...
## Claude Code
1. A project's instructions live in `PROJECT/AGENTS.md`, and `PROJECT/.claude/CLAUDE.md` carries that one file so every harness reads one source. An `@../AGENTS.md` import there is expanded only for a session started at the repository root; a symlink to `../AGENTS.md` is read from anywhere.
2. A skill the model loads takes a gerund — verb-ing plus object, `updating-wiki-pages`, `delegating` — because it names an operation the model picks up. A skill only a person types takes a verb phrase and hides from the model — `run`, `verify`, `clean-sessions`, with `disable-model-invocation: true` (add `model-invocable: false` while board's command reader still keys on it) — because a command is an order given, not an operation offered.
