...
- Writing and language: **ground rule for every document: keep it short, and separate the
...
## Claude Code
1. A project's instructions live in `PROJECT/AGENTS.md`, and `PROJECT/.claude/CLAUDE.md` imports it with `@../AGENTS.md` so every harness reads one source — the workspace root included. The shim is what loads it: a bare `AGENTS.md` is read by nothing. The import expands from the repository root, from a subdirectory, and from an ancestor alike, so a session inside an entry carries that entry's file and every ancestor's above it (probed 2026-08-21).
2. A skill the model loads takes a gerund — verb-ing plus object, `updating-wiki-pages`, `delegating` — because it names an operation the model picks up. A skill only a person types takes a verb phrase and hides from the model — `verify`, `deploy`, `clean`, with `disable-model-invocation: true` — because a command is an order given, not an operation offered.
...
2. A rule on writing, visualizing, describing, or explaining goes to the `writing` skill, `~/workspace/.claude/skills/writing/` — into the `references/` file whose subject covers it, because that skill is the only thing that reads such rules.
...
5. A repository-specific working gotcha — how to build or test *that* codebase — goes to that repository's `AGENTS.md`, edited in a worktree and committed like any other change. A debugging pitfall specific to a project's technology or conventions goes instead to that project pool's `pitfalls.md`, per the debugger constitution's own filing rule (`~/.claude/agents/debugger.md`) — never to an `AGENTS.md` or `docs/`.
6. A product, architecture, or verification truth about a repository goes to that repository's own source of truth (its `docs/` or spec), never to its `AGENTS.md`.
