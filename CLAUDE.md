...
## Claude Code
1. A project's instructions live in `PROJECT/AGENTS.md`, and `PROJECT/.claude/CLAUDE.md` imports it with `@../AGENTS.md` so every harness reads one source — the workspace root included. The shim is what loads it, and every ancestor's shim loads too, so an entry's session carries the workspace file above its own.
