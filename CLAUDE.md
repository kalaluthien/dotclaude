...
Write an item's body as indented field bullets under the head line, and put nothing after the bold title on the head line itself. The fields, in this order: `what:` (the work in one plain sentence), `why:` (why the item exists), `next:` (the next step and who takes it), then optional `scope:` (what the worker touches) and `evidence:` (the paths, commits, and dates that back the other fields). One row serves two readers at once: the owner reviewing cards reads `what`, `why`, `next`; the session picking the item up reads `what`, `scope`, `evidence`. Each field is bounded by `item.body.field_max_chars`, and a bound is a ceiling, never a target — the superseded rule's single 300-character budget taught sessions to fill it, packing every row dense instead of keeping it plain, so each fact now has its own slot and the fewest plain words win. Use plain words and name the actor, because the row is read by the owner as a card on a phone and not by the session that wrote it. Rewriting a row preserves every fact, marker, date, tag, and reference, and changes the wording alone. (Owner decision, 2026-08-15, superseding the 2026-08-12 three-sentence 300-character body, whose options and kill reasons stay in `~/workspace/docs/agent-backlog-writing.html`; the packed-row evidence is in the workspace pool's `feedback-backlog-wording`.)
